...
const USE_RELIABLE_MESSAGING := false
...
func _handle_message_from_client_reliable(message):
	LogsAndMetrics.add_sample(RECV_INTERVAL_USEC, Time.get_ticks_usec())
	emit_signal("received_client_message", multiplayer.get_remote_sender_id(), message)
...
func _handle_message_from_server_reliable(message):
	emit_signal("received_server_message", message)

func send_message_to_client(client_network_id, message):
	if USE_RELIABLE_MESSAGING:
		_handle_message_from_server_reliable.rpc_id(client_network_id, message)	
	else:
...
func send_message_to_server(message):
	...
	if USE_RELIABLE_MESSAGING:
		_handle_message_from_client_reliable.rpc_id(SERVER_NETWORK_ID, message)
	else:
